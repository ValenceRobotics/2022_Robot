...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    m_arm.setDefaultCommand(ArmCommands.armTriggerOperationPID(m_arm, m_xboxController));
  }
  ...
  private void configureButtonBindings() {
    ...
    (new JoystickButton(m_xboxController, Constants.OI.kArmUpButton))
      .whenPressed(new InstantCommand(()->m_climber.setSpeed(.8), m_climber));

    (new JoystickButton(m_xboxController, Constants.OI.kArmDownButton))
      .whenPressed(new InstantCommand(()->m_climber.setSpeed(-.4), m_climber)
      .andThen(new InstantCommand(() -> m_climber.engageHolding())));

    
    (new JoystickButton(m_xboxController, Constants.OI.kClimberLeftDownSlow))
      .whileHeld(new InstantCommand(() -> m_climber.disableHolding(), m_climber)
      .andThen(ClimberCommands.climberLowerSlow(m_climber, true)))
      .whenReleased(ClimberCommands.climberStop(m_climber, true));

    (new JoystickButton(m_xboxController, Constants.OI.kClimberRightDownSlow))
      .whileHeld(new InstantCommand(() -> m_climber.disableHolding(), m_climber)  
      .andThen(ClimberCommands.climberLowerSlow(m_climber, false)))
      .whenReleased(ClimberCommands.climberStop(m_climber, false));
